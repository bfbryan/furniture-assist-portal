    ...
    <div className="min-h-screen bg-[#F7F5F1]">
      ...
      <header className="fa-detail-header" style={{ background: 'white', borderBottom: '1px solid #EDE9E1', padding: '12px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
          ...
          {/* Client receipt, in the header beside the status. Ben: "need to add
              to the client ID page - put up top in the status card."

              WHY HERE. This bar is the only container on the page that shows
              the referral's status, sits at the top, and needs no scrolling to
              reach on any screen — it is sticky from 1280 up, and on a phone it
              is the first thing under the shell's navy bar. The right-hand
              cards ("Referral Details", "Appointment") are above the fold on a
              desktop but drop below the whole left column once the grid stacks,
              which is the opposite of what was asked for.

              It is also the pattern already in use: the internal detail page
              (app/dawson/referrals/[id]/page.tsx) puts Appt Slip and Data Page
              in exactly this spot, right of the status pill, and this header
              was built to mirror that one. So this is the second use of an
              existing arrangement, not a new one.

              Sizing follows THIS header's buttons (7px/16px, 12px text) rather
              than the internal page's slightly larger ones, so it lines up with
              Reschedule / Cancel beside it. White with a teal glyph marks it as
              a document to open rather than an action that changes something —
              same distinction the internal page draws.

              Gated on the URL alone, not on status === 'Completed'. The receipt
              is written by the post-visit cron, so in practice only a completed
              referral has one and the extra test buys nothing; if a receipt
              does exist, the agency should be able to reach it. Same rule the
              internal page applies to the appointment slip. Note a completed
              referral has no action buttons at all, so on the records where
              this shows it sits immediately after the status pill. */}
          {referral.clientReceiptUrl && (
            <a href={referral.clientReceiptUrl} target="_blank" rel="noreferrer"
              style={{ padding: '7px 16px', borderRadius: '7px', border: '1px solid #EDE9E1', background: 'white', color: '#2A7F6F', fontFamily: 'var(--font-montserrat)', fontWeight: 700, fontSize: '12px', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/>
              </svg>
              Client Receipt
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <Card accent={READ_ACCENT} title="Appointment">
            ...
            {/* Generated by the client-receipt cron into an Airtable
                ...
                DELIBERATELY KEPT alongside the copy now in the page header —
                this is not a duplication left behind by that change. Ben asked
                for the receipt to be ADDED up top, not moved, and this card is
                where the appointment's other documents live, so removing it
                would take the receipt out of the one place on the page that
                collects them. Easy to drop if the pair reads as noise on the
                preview; see the note in the header for the placement argument. */}
