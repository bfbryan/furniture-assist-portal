...
export async function getStaffWithDetails(staffId: string) {
  ...
  //
  // {Referring Staff Link} is a single link; equality against the record id
  // via FIND("recXXX", ARRAYJOIN(...)) is the safe pattern that works with
  // Airtable's formula language for link fields.
  const [agency, referralData] = await Promise.all([
    ...
    airtableFetch(
      ...
      `?filterByFormula=${encodeURIComponent(
        `FIND("${staffId}", ARRAYJOIN({Referring Staff Link}, ","))`,
      )}&sort[0][field]=Referral%20Date&sort[0][direction]=desc`,
    ),
  ])
