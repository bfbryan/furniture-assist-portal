...
// can actually follow.
//
// THE BUG THIS EXISTS TO CLOSE. Two invite paths ask Clerk for a sign-in token
// and get back the same response — a raw `token` plus a ready-made `url` — and
// until now they disagreed about which half to use:
//
//   Agency admin welcome (POST /api/dawson/agencies/[id]/invite)
//       sends the RAW token, and its Airtable template hard-codes
//       https://portal.furnitureassist.com/sign-in?__clerk_ticket={{token}}
//       around it. The recipient lands in the portal. Correct.
//
...
//       That is exactly what Ben reported.
//
...
// The origin below is a constant for the same reason the admin template hard-
// codes it: the portal has exactly one address, an invite email is read long
// after the deploy that sent it, and a preview deploy must not mail out links
// to itself. One constant, one place to change it.
//
// The query parameter is Clerk's own: `__clerk_ticket` is what
// app/(public)/sign-in consumes to complete a ticket sign-in.

/** Where invite emails send people. Matches the admin welcome template. */
